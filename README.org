...
A function applied to each cell value (after =:key-fn=, before
escaping) to produce the display string. Defaults to =str=, with
special handling for =nil= (empty string) and =java.util.Date=. Must
return a string.
...
A function applied to the column's =:name= (or =:key-fn=) to produce
the header string, instead of the default titleization. Must return a
string.
...
Like =:fancy=, but with double-line borders. =:double-grid= is also
available if you'd like separators between data rows.
